            ...
            <div class="row">
                ...
                @if($sections)
                    @foreach($sections as $station_name => $section)
                        ...
                        @php $total_score = []; @endphp

                        @foreach($section->datasets as $dataset)
                            @php
                                $total_score[] = $dataset->values[0];
                            @endphp
                        @endforeach
                        ...
                        <div class="col-md-12">
                            <div class="panel panel-default">
                                <div class="panel-heading" style="padding: 15px;">
                                    {{$station_name}} score chart  - Total : {{array_sum($total_score)}}
                                </div>
                                <div class="panel-body">
                                     {!! $section->container() !!}
                                </div>
    ...
    @if($sections && auth()->user()->is_cmo)
        ...
        <script>
            ...
            barOnClick = (evt, title) => {
                ...
                let section_weight = typeof bar.payload == 'object' ? bar.payload[0].section_weight : 0;
